...
function savecache($name = '', $id='')
{
    if ($name=='System') {
        ...
        $Model = db ($name);
        $list = $Model->select ();
        $data = convert_arr_kv($list,'name','value');
        cache($name, $data);
    }  else {
        $Model = db ($name);
 ...
 * @return bool
 */
function dir_file_delete($runtime_path, $suffix=false)
{
    $handle = opendir($runtime_path);
    while (($item = readdir($handle)) !== false) {
        if ($item != '.' and $item != '..') {
            if (is_dir($runtime_path . '/' . $item)) {
                dir_file_delete($runtime_path . '/' . $item);
            } else {
                if ($suffix) { // 限定后缀
                    $item_suffix = fileext($item);
                    if ($suffix == $item_suffix) {
                        if (!unlink($runtime_path . '/' . $item)) {
                            die('error!');
                        }
                    }
                } else {
...
/**
 * 用于去除AngularJs中option value 带有string:、number:内容
 * @param array $data
 * @param array $name
 * @return string
 */
function get_select_value($data, $name)
{
    foreach ($name as $k => $v) {
        $value =explode(':',$data[$v]);
        $data[$v] = isset( $value[1])?$value[1]:'';
    }
    return $data;
}
 ...
 * @param bool $refresh
 */
function js_alert($content='', $url=false, $back=false, $die=true, $refresh=false)
{
    $do = 'alert("'.$content.'");';
    if ($url) {
...
/**
 * 用于去除AngularJs中单个option value 带有string:、number:内容
 * @param $content
 * @return string
 */
function get_option_value($content)
{
 ...
 */
function the_function_price($goods_info)
{
    $vip_info = db('user_vip')
        ->where(['uid' => session('user.id')])
        ->where('expires_time', '>', time())
        ->find();
    if ($vip_info) {
        return $goods_info['member_price'];
    }
 ...
 * @return array
 */
function get_start_end_time($times)
{
    $time_array = explode('~', $times);
    return [
        'start' => strtotime($time_array[0]),
 ...
 * @return string
 */
function set_start_end_time($start_time, $end_time)
{
